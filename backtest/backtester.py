# /root/ricealert/backtest/backtester.py (v3 - Portfolio Backtesting)
...
# --- CẤU HÌNH CHO BACKTEST DANH MỤC ---
CONFIG = {
    "SYMBOLS": [
        "ETHUSDT", "AVAXUSDT", "INJUSDT", "LINKUSDT", 
        "SUIUSDT", "FETUSDT", "TAOUSDT"
    ],
    "INTERVAL": "1h",
    "TOTAL_DAYS_TO_TEST": 90,
    "INITIAL_CAPITAL": 10000.0,
    "MAX_ACTIVE_TRADES": 4, # Giới hạn số lệnh mở cùng lúc
    "TRADE_AMOUNT_PERCENT": 0.1, # Mỗi lệnh đi 10% vốn tại thời điểm vào lệnh
    "COMMISSION_RATE": 0.0004,
    "ADVISOR_BUY_SCORE_THRESHOLD": 7.0, # Nâng ngưỡng lên để chọn lọc hơn
}
...
def load_all_historical_data(symbols, interval, days):
    """Tải dữ liệu lịch sử cho TẤT CẢ các symbols."""
    all_data = {}
    print("--- Bắt đầu quá trình tải và xử lý dữ liệu cho danh mục ---")
    for symbol in symbols:
        cache_file = os.path.join(CACHE_DIR, f"{symbol}-{interval}-{days}d.parquet")
        if os.path.exists(cache_file):
            print(f"  - Đọc {symbol} từ cache...")
            df = pd.read_parquet(cache_file)
        else:
            print(f"  - Đang tải {symbol} từ API...")
        ...
        if not df.empty:
            # Tính toán các features cần cho AI ngay từ đầu
...
def load_all_ai_models(symbols, interval):
    """Tải tất cả mô hình AI cần thiết."""
    all_models = {}
    ...
    for symbol in symbols:
        try:
            ...
            print(f"  - Tải model cho {symbol} thành công.")
        except FileNotFoundError:
            print(f"  - CẢNH BÁO: Không tìm thấy model AI cho {symbol}. Sẽ bỏ qua coin này.")
    print("--- Hoàn thành tải mô hình AI. ---")
    return all_models

def run_portfolio_backtest(all_data, all_models):
    """Hàm chính thực hiện backtest trên toàn bộ danh mục."""
    print(f"\n--- Bắt đầu Portfolio Backtest ---")
    capital = CONFIG['INITIAL_CAPITAL']
    trade_history = []
    active_trades = {} # Dùng dict để lưu các lệnh đang mở, key là symbol

    # Tạo một master index thời gian từ tất cả các dataframe
    ...
    print(f"-> Mô phỏng trên {len(master_index)} đơn vị thời gian (giờ).")

    # Vòng lặp chính theo từng đơn vị thời gian
    for timestamp in master_index:
        # 1. Kiểm tra và đóng các lệnh đang hoạt động
        for symbol in list(active_trades.keys()):
            if timestamp in all_data[symbol].index:
                ...
                if status:
                    ...
                    trade.update({
                        'exit_price': current_price, 'exit_time': timestamp,
                        'pnl_percent': pnl * 100, 'pnl_usd': net_pnl_usd, 'status': status
                    })
                    trade_history.append(trade)
                    del active_trades[symbol] # Xóa lệnh khỏi danh sách active
                    # print(f"   -> {timestamp} | {status} Closed: {symbol}, PnL: {net_pnl_usd:+.2f}$")

        # 2. Tìm kiếm cơ hội mới nếu còn "chỗ"
        if len(active_trades) < CONFIG['MAX_ACTIVE_TRADES']:
            # Quét qua tất cả các coin để tìm tín hiệu
            for symbol in all_data.keys():
                # Chỉ tìm lệnh mới nếu chưa có lệnh cho coin này
                if symbol not in active_trades and timestamp in all_data[symbol].index:
                    ...
                    if symbol in all_models:
                        ...
                        if not X.isnull().values.any():
                            ...
                            ai_data_for_advisor = {"prob_buy": prob_buy, "pct": 0} # Đơn giản hóa

                    # Gọi hàm advisor
                    decision_data = get_advisor_decision(symbol, CONFIG['INTERVAL'], indicators, {}, ai_data_override=ai_data_for_advisor)
                    
                    if decision_data['decision_type'] == 'OPPORTUNITY_BUY' and decision_data['final_score'] >= CONFIG['ADVISOR_BUY_SCORE_THRESHOLD']:
                        amount_to_invest = capital * CONFIG['TRADE_AMOUNT_PERCENT']
                        commission = amount_to_invest * CONFIG['COMMISSION_RATE']
                        capital -= commission

                        active_trades[symbol] = {
                            'symbol': symbol, 'entry_price': indicators['price'], 'entry_time': timestamp,
                            'tp': decision_data['combined_trade_plan']['tp'], 'sl': decision_data['combined_trade_plan']['sl'],
                            'amount_usd': amount_to_invest, 'advisor_score': decision_data['final_score']
                        }
                        # print(f"[*] {timestamp} | NEW BUY: {symbol} at {indicators['price']:.4f} (Score: {decision_data['final_score']:.1f})")
                        
                        # Nếu đã đủ số lệnh tối đa thì dừng quét
                        if len(active_trades) >= CONFIG['MAX_ACTIVE_TRADES']:
                            break

    return trade_history, capital

# (Hàm generate_report không thay đổi, giữ nguyên như cũ)
def generate_report(trade_history, final_capital):
    print("\n\n--- BÁO CÁO KẾT QUẢ BACKTEST DANH MỤC ---")
    ...
    avg_pnl_usd = df_trades['pnl_usd'].mean()
    ...
    print(f"Tỷ lệ thắng          : {win_rate:.2f}% ({win_trades} Thắng / {loss_trades} Thua)")
    print(f"Lãi/lỗ trung bình    : ${avg_pnl_usd:,.2f} / lệnh")
    print(f"Lệnh thắng lớn nhất   : ${max_win_usd:,.2f}")
    print(f"Lệnh thua lớn nhất    : ${max_loss_usd:,.2f}")
    print("-" * 35)
    
    # Thống kê theo từng coin
    pnl_by_symbol = df_trades.groupby('symbol')['pnl_usd'].sum().sort_values(ascending=False)
    print("PnL theo từng Symbol:")
    print(pnl_by_symbol)
    
    report_file = os.path.join(current_dir, f"backtest_report_PORTFOLIO_v3.csv")
    df_trades.to_csv(report_file, index=False)
    print(f"\nĐã lưu chi tiết các giao dịch vào: {report_file}")
...
if __name__ == "__main__":
    ...
    # Lọc ra những coin có cả dữ liệu và model
    valid_symbols = list(all_models.keys() & all_data.keys())
    print(f"\nSẽ chạy backtest trên các symbols hợp lệ: {valid_symbols}")
    ...
    if filtered_data:
        ...
        generate_report(history, final_capital)
    else:
        print("Không có dữ liệu hoặc mô hình hợp lệ để chạy backtest.")
