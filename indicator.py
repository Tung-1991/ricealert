# /root/ricealert/indicator.py (PHIÊN BẢN FINAL - ĐÃ SỬA LỖI ATR)
import pandas as pd
import ta
import requests
...
def calculate_indicators(df: pd.DataFrame, symbol: str, interval: str) -> dict:
    ...
    if len(df) < 51:
        ...
        return {
            ...
            "ema_20": 0.0, "ema_50": 0.0, "trend": "sideway",
            ...
            "atr": 0.0, # <-- Sửa lỗi: Thêm giá trị mặc định cho ATR
            "atr_percent": 2.0, "reason": "Thiếu dữ liệu"
        }
    ...
    # ---- SỬA LỖI ATR: Tính và trả về cả atr_value và atr_percent ----
    atr_percent = 2.0
    atr_value = 0.0 # Giá trị mặc định
    try:
        atr_series = ta.volatility.average_true_range(df["high"], df["low"], df["close"], window=14)
        atr_value = atr_series.iloc[closed_candle_idx] # Lấy giá trị ATR gốc
    ...
    if not np.isnan(fib_0_618) and abs(fib_0_618 - price) / price < 0.02:
    ...
    if len(df) >= abs(closed_candle_idx) + 1:
        ...
        curr_c = df.iloc[closed_candle_idx]
    ...
    result = {
        ...
        "atr": atr_value, # <-- SỬA LỖI: Thêm giá trị ATR gốc vào kết quả
